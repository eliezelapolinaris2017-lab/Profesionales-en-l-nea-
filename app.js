...
/* === NUEVO: helpers Drive + selección de foto === */
// Convierte /file/d/ID/view o ?id=ID en uc?export=view
...
// Elige la mejor fuente de imagen (thumb > pública > raw convertida)
function pickPhoto(p){
  const thumb = p.foto_thumb || p.photo_thumb || null;
  const pub   = p.foto_publica || p.photo_public || null;
  const raw   = p.foto || p.photo || null;
  return thumb || pub || fixDriveUrl(raw) || null;
}
...
function unifyPro(p) {
  ...
  // campos de foto (incluye nuevos si existen)
  const foto_raw   = p.foto ?? p.photo ?? null;
  const foto_pub   = p.foto_publica ?? p.photo_public ?? null;
  const foto_thumb = p.foto_thumb   ?? p.photo_thumb   ?? null;
  ...
  return {
    ...
    name, phone, category: cat, town, services, bio,
    // guardamos todo para la estrategia pickPhoto
    foto: foto_raw, foto_publica: foto_pub, foto_thumb
  };
}
...
function renderResults(list){
  ...
  list.forEach(p=>{
    ...
    // === NUEVO: decidir la imagen final para <img> ===
    const photoSrc = pickPhoto(p);
        ...
        <div class="avatar">
          ${
            ...
            ? `<img referrerpolicy="no-referrer" loading="lazy"
                     ...
                     onerror="this.onerror=null; this.src='img/avatar-generico.png';"
