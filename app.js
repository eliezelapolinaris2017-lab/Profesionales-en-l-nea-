...
/* ---- Helpers sólidos para fotos de Drive ---- */
function extractDriveId(urlOrId) {
  if (!urlOrId) return null;
  // ID de Drive típico (25+ chars)
...
async function loadDB() {
  ...
  console.table(pros.map(p => ({ name:p.name, phone:p.phone, category:p.category, town:p.town, photo:p.photo })));
}
...
function renderResults(list){
  ...
  list.forEach(p=>{
    ...
    // Estructura base: avatar con iniciales por defecto
    el.innerHTML = `
      <div class="row">
    ...
    // Si hay foto, la precargamos y reemplazamos las iniciales solo si carga
    if (p.photo) {
      const url = p.photo;
      preloadImage(url).then(()=>{
        ...
        // Se mantienen las iniciales; si prefieres, descomenta el fallback a logo:
        // const avatar = el.querySelector('.avatar'); if (avatar) avatar.innerHTML = '<img src="logo.png" style="width:100%;height:100%;object-fit:cover;border-radius:12px">';
      });
    }
  });
