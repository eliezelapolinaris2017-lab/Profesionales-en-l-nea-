...
/* === Helpers de imágenes (Drive) === */
// Convierte /file/d/ID/view o ?id=ID → uc?export=view
function fixDriveUrl(url){
  ...
  if(m && m[1]) return `https://drive.google.com/uc?export=view&id=${m[1]}`;
  return url; // ya venía directa
}
function extractIdFromUrl(url){
  if(!url) return null;
  let m = String(url).match(/\/file\/d\/([a-zA-Z0-9_-]+)/); if(m) return m[1];
  m = String(url).match(/[?&]id=([a-zA-Z0-9_-]+)/);        if(m) return m[1];
  m = String(url).match(/\/d\/([a-zA-Z0-9_-]+)\//);        if(m) return m[1];
  return null;
}
function makeThumbFromRaw(url, size=512){
  const id = extractIdFromUrl(url);
  return id ? `https://drive.google.com/thumbnail?id=${id}&sz=w${size}-h${size}` : null;
}
// Fallback embebido (no necesitas archivos extra)
const AVATAR_PLACEHOLDER =
'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="256" height="256" viewBox="0 0 256 256"><rect width="256" height="256" rx="24" fill="%23111"/><circle cx="128" cy="96" r="44" fill="%23333"/><rect x="52" y="152" width="152" height="56" rx="28" fill="%23333"/></svg>';

/* ======== Normalización de cada pro ======== */
function unifyPro(p) {
  ...
  // ⚠️ Foto: prioriza campos públicos del JSON; si no existen, convierte la URL cruda
  const raw   = p.foto ?? p.photo ?? null;
  const photo_public = p.photo_public ?? p.foto_publica ?? null;
  const photo_thumb  = p.photo_thumb  ?? p.foto_thumb   ?? null;
  const photo = photo_public || photo_thumb || fixDriveUrl(raw) || null;
  ...
  return {
    ...
    name, phone,
    category: cat, town, services, bio,
    photo,            // <- este es el que usaremos en el render
    _raw: raw,        // guardo por si necesitamos hacer thumbnail como 2º intento
    _thumb: photo_thumb
  };
}
...
function renderResults(list){
  ...
  list.forEach(p=>{
    ...
    // fuente principal de imagen
    const primary = p.photo;
    // si llegara a fallar, intentamos thumbnail (si no lo teníamos ya)
    const second  = p._thumb || makeThumbFromRaw(p._raw);
        ...
        <div class="avatar">
          ${
            ...
            ? `<img referrerpolicy="no-referrer" loading="lazy"
                     src="${primary}"
                     alt="${p.name||'Foto de perfil'}"
                     onerror="if(!this.dataset.fallback && '${second}'){ this.dataset.fallback=1; this.src='${second}'; } else { this.onerror=null; this.src='${AVATAR_PLACEHOLDER}'; }"
