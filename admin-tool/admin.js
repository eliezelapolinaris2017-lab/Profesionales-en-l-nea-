...
let DB = { items: [] }; // {nombre, telefono, categoria, pueblo, servicios[], descripcion, foto, whatsapp, activo}
...
// Config opcional
if (j.config) {
CFG.bg_mode = j.config.bg_mode || CFG.bg_mode;
CFG.bg_urls = j.config.bg_urls || CFG.bg_urls;
CFG.bg_opacity = parseFloat(j.config.bg_opacity ?? CFG.bg_opacity);
CFG.bg_blur = parseInt(j.config.bg_blur ?? CFG.bg_blur,10);
}
...
/** ====== POST SIMPLE (UNO POR UNO) ====== */
async function postOneProfessional(item){
...
fd.append('pueblo', item.pueblo||'');
fd.append('servicios', (item.servicios||[]).join(', '));
fd.append('descripcion', item.descripcion||'');
fd.append('foto', item.foto||'');
fd.append('activo', item.activo||'');
fd.append('whatsapp', item.whatsapp||'');

const r = await fetch(ENDPOINT, { method:'POST', body: fd }); // SIN headers → no hay preflight
const j = await r.json();
if(!j.ok) throw new Error(j.error||'Error guardando 1 registro');
return j;
}

async function pushItems(){
if(!ENDPOINT) return alert("Configura el endpoint en la pestaña Config.");
if(!DB.items.length) return alert("No hay profesionales en base local.");
try{
let ok = 0;
for (const it of DB.items) {
await postOneProfessional(it);
ok++;
}
alert(`Profesionales enviados: ${ok}`);
}catch(err){ alert("Error: "+err.message); }
}
...
if(!DB.items.length) return alert("No hay profesionales en base local.");
try{
// 1) Subir profesionales (uno por uno)
await pushItems();
// 2) Guardar config de fondo
...
fd.append('bg_mode', CFG.bg_mode);
fd.append('bg_urls', CFG.bg_urls);
fd.append('bg_opacity',String(CFG.bg_opacity));
fd.append('bg_blur', String(CFG.bg_blur));
try{
const r = await fetch(ENDPOINT, { method:'POST', body: fd }); // SIN headers
const j = await r.json();
...
if(!v) return alert("Pega tu URL /exec de Apps Script (puedes usar corsproxy).");
